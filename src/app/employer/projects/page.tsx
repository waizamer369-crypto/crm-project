...
import { Plus, Users, CheckCircle, Clock, LogOut, Briefcase, X, Mail, Search, Trash2, ChevronDown, ChevronUp, Filter, UserPlus, Shield, User, Phone, Star, Share2, Calendar } from "lucide-react"
...
function ProjectStatusDropdown({ projectId, currentStatus, onStatusChange }: {
  ...
  const handleSelect = async (statusValue: string) => {
    ...
    setUpdating(false)
    if (res.ok) {
      onStatusChange(projectId, statusValue)
    }
  }
...
export default function EmployerProjects() {
  ...
  const createTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!targetProjectId) {
        setEmailError("Please select a project.")
        setCreating(false)
        return
      }
      ...
      if (!res.ok) {
        setEmailError(data.error || "Failed to create task")
        setCreating(false)
        return
      }
    ...
    try {
      const empRes = await fetch("/api/employees", {
        ...
        body: JSON.stringify({
          ...
          workingHoursPerDay: memberForm.workingHoursPerDay
        })
      })
      ...
      if (!empRes.ok) {
        setMemberError(empData.error || "Failed to create employee")
        setCreating(false)
        return
      }
      ...
      if (memberForm.projectId) {
        const projRes = await fetch("/api/projects", {
          ...
          body: JSON.stringify({
            ...
            email: memberForm.email,
            role: memberForm.projectRole
          })
        })

        const projData = await projRes.json()
        if (!projRes.ok) {
          setMemberError(projData.error || "Employee created but failed to add to project")
          setCreating(false)
          fetchProjects()
          return
        }
...
<nav className="space-y-1">
              <Link href="/employer/projects" className="flex items-center gap-3 px-4 py-3 bg-blue-50 text-blue-700 rounded-xl font-medium">
              ...
              <Link href="/employer/calendar" className="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-50 rounded-xl font-medium transition">
          ...
          <div className="p-8">
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedProject !== "all" && (
                  <button 
                    onClick={() => { setSelectedProject("all"); setExpandedProject(null); }}
                        ...
                        {isExpanded && (
                          <div className="p-5 bg-slate-50/50">
                                ...
                                {project.members && project.members.length > 0 ? (
                                  project.members.map((m, idx) => (
                                    <span key={idx} className="flex items-center gap-1.5 px-2.5 py-1 bg-white rounded-lg border border-slate-200 text-sm">
                                      ...
                                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${roleColors[m.role || "MEMBER"] || roleColors.MEMBER}`}>
                                        {m.role || "MEMBER"}
                                      </span>
                                    </span>
                                  ))
                            ...
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <button 
                                onClick={(e) => { e.stopPropagation(); setTaskForm({...taskForm, projectId: project.id}); setShowTaskModal(true); }}
                                    ...
                                    <div className="flex items-start justify-between">
                                      <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-slate-500">
                                          ...
                                          {task.assignee && (
                                            <span className="flex items-center gap-1"><Mail className="w-3.5 h-3.5" />{task.assignee.name} ({task.assignee.email})</span>
                                          )}
                                        </div>
                                      </div>
                                      <button 
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <form onSubmit={createTask} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Assign to (Employee Email)</label>
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                  <input type="email" value={taskForm.assigneeEmail} onChange={(e) => { setTaskForm({...taskForm, assigneeEmail: e.target.value}); setEmailError(""); }}
                    className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="john@company.com" required />
                </div>
                {emailError && (
                  <p className="mt-2 text-sm text-red-600 flex items-center gap-1.5">
                    <Search className="w-4 h-4" />{emailError}
                  </p>
                )}
              </div>
              <div className="flex gap-3 pt-2">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              ...
              <button onClick={() => { setShowAddMemberModal(false); setMemberError(""); setMemberSuccess(""); }} className="text-slate-400 hover:text-slate-600 transition">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <form onSubmit={addMember} className="p-6 space-y-5">
              {memberError && (
                <div className="p-4 bg-red-50 border border-red-200 text-red-600 rounded-xl text-sm">{memberError}</div>
              )}
              {memberSuccess && (
              ...
              <div>
                ...
                <input
                  type="number" min={1} max={24} value={memberForm.workingHoursPerDay}
                    ...
                    {(["MEMBER", "ADMIN", "VIEWER"] as const).map((r) => (
                      <button key={r} type="button" onClick={() => setMemberForm({ ...memberForm, projectRole: r })}
                        className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 transition font-medium ${
                          memberForm.projectRole === r ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 hover:border-slate-300 text-slate-600"
                        }`}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(["EMPLOYEE", "EMPLOYER"] as const).map((r) => (
                      <button key={r} type="button" onClick={() => setMemberForm({ ...memberForm, role: r })}
                        className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 transition font-medium ${
                          memberForm.role === r ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 hover:border-slate-300 text-slate-600"
                        }`}>
